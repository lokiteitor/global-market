...
### 5.13 Nota de materialización (Incremento 10 — red eléctrica regional, Fase 3)

El décimo incremento activa el **último elemento diferido del diseño**: la red eléctrica regional de GDD 5.8, conservada íntegra como especificación desde la v1.2. Las decisiones de convivencia con el combustible in situ están registradas en **ADR-025** (convivencia por receta, puja de demanda para el recorte por prioridad inversa, conectividad de pool regional, líneas sin concesión); dos migraciones (`0017_electric_grid_enums`, no-transaction; `0018_electric_grid`, transaccional) y el contrato **v1.6.0** (aditivo). Tres piezas en tres superficies, con las fronteras del §5.1 intactas:

- **Activos físicos ⇒ `internal/world/power` (handlers en el gateway).** Subpaquete del contexto world (comparte `world/sqlcgen`): **líneas de transmisión** (`world.power_lines`: huella `LineString` SRID 0 validada server-side **íntegra dentro de una región** — sin interconexiones interregionales, GDD 22 —, coste por longitud al sink, **sin concesión de suelo**: su mantenimiento por km/día-sim cumple el papel anti-acaparamiento del canon), **ofertas de generación** (`power_offers`: sin oferta la central no participa) y **pujas de consumo** (`power_bids`: prioridad inversa del recorte y techo personal), más las lecturas del contrato (catálogo de líneas, histórico del spot, despachos propios). Las **centrales** son `building_types` normales (concesión, huella, mantenimiento, cascada de insolvencia estándar) con sus parámetros de generación en `world.power_plant_types`; **no tienen recetas** — su generación la gobierna el despacho del spot. El emplazamiento de las hidroeléctricas usa la regla server-side nueva `requires_biome` (hoy `coast`; los ríos del worldgen siguen pendientes).
- **Mantenimiento de líneas ⇒ `internal/world/enforcement` (engine).** Cuarto barrido del motor de insolvencia (`SweepPowerLineMaintenance`), patrón exacto de edificios: cobra **solo lo disponible** (`II_POWER_LINE_MAINT_PER_KM_DAY` × km × días vencidos), cada día se salda exactamente una vez, los impagados degradan (`II_DEGRADE_PCT_PER_SIM_DAY`) y a ≤ `II_ABANDON_CONDITION_PCT` la línea pasa a **`abandoned`: deja de conducir** (terminal; sin embargo ni subasta — el valor de una línea es su trazado) emitiendo `power_line.abandoned`.
- **Tick del mercado spot ⇒ `PowerWorker` en `internal/balancer` (engine).** El GDD 18.1 asigna el tick al Economy Balancer. Barrido wall-clock (`II_POWER_SPOT_SWEEP_INTERVAL`) que liquida, por cada región con líneas operativas, el **bucket** de sim-time vencido (`II_POWER_SPOT_INTERVAL_SIM`, default 1 hora-sim; idempotente por PK `(region_id, tick_sim)`; los buckets perdidos **no se recuperan**: la energía no es almacenable y un tick perdido no tiene efecto contable). Cada región en **una tx SERIALIZABLE**: casación pura (orden de mérito ascendente; demanda por puja descendente con **rotación** — entre pujas iguales cae primero el recortado menos recientemente —; consumidores todo-o-nada, generadores divisibles; térmicas limitadas por su combustible en ambos planos: **sin combustible no despachan**), **un asiento `power_spot`** con las cajas de los consumidores servidos al débito y las de los generadores despachados al crédito **al precio de cierre uniforme** (redistribución cash→cash: ni faucet ni sink), un asiento `consumption` contra `world_source` por térmica despachada (ADR-022, físico+espejo en la misma tx), el plano físico del tick (`power_spot_ticks`/`power_dispatches`), la pausa de los no servidos (**`paused_no_power`**, evento `power.curtailed` con reason `curtailed`/`insolvent`) y la reanudación de los servidos (`powered_until_sim = tick + 1,5×intervalo`), con `power.spot_cleared` por el outbox en la misma tx. La **insolvencia excluye sin deuda** (GDD 5.9: presupuesto por corporación, puja×energía conservador; el trigger de no-negatividad respalda). El motor de producción solo añade una guarda: cerrar un lote de receta eléctrica exige `powered_until_sim > simNow`, y `tryResume` reanuda si la receta dejó de ser eléctrica o la cobertura sigue vigente.

**Sin imports cruzados nuevos**: el `PowerWorker` escribe cross-schema con queries sqlc propias del Balancer ("la frontera es de código Go, no de esquema", como `city_consumer`), y la integración hacia fuera va por el outbox — eventos nuevos `power_line.created` (world/power), `power_line.abandoned` (world/enforcement), `power.spot_cleared` y `power.curtailed` (balancer), todos contratos de evento fijos. La **convivencia** con el combustible in situ es **por receta y opt-in** (ADR-025 §1): el día de la activación ningún edificio cambia de comportamiento; el arbitraje combustible↔electricidad es gameplay nuevo. Detalle contable y parámetros en la sección *v1.10* de `documentacion_base_de_datos.md`; la nota de implementación de `gdd.md` §5.8 registra las tres decisiones.
...
│   │   ├── world/               # World Simulation Service: catalog/land/buildings/production (Incremento 2) + fleet (Incremento 3: flota, cargamentos, motor de tránsito; Incremento 7: tránsito multimodal rail/sea con transbordo en terminal) + enforcement (Incremento 6a: cascada de insolvencia — mantenimiento, degradación, embargo; Incremento 10: mantenimiento de líneas eléctricas) + power (Incremento 10, ADR-025: líneas de transmisión, ofertas/pujas y lecturas del spot) + sqlcgen del contexto
...
│   │   ├── balancer/            # Economy Balancer Service (Incremento 6b): agente decisor de ciudades (DemandWorker: curvas de demanda + niveles + buys por PORT), Consumer city_consumer (consumo urbano final) y AnalyticsWorker (analítica macro + fórmula laboral + lazo fiscal) + PowerWorker (Incremento 10, GDD 18.1: tick del mercado spot eléctrico por región) + sqlcgen propio
...
- **Elementos diseñados pero diferidos** (especificación conservada, activación por fase): extracción multi-proceso (medida) y el **pathfinding jerárquico HPA*** del Logistics Service (optimización por escala, no cambio de arquitectura: la interfaz `Planner` lo deja listo; el Dijkstra plano ponderado por congestión de la Fase 1 lo hace innecesario mientras el grafo sea pequeño — sin ADR). La **red eléctrica regional** (Fase 3) está **materializada** en el Incremento 10 (ver §5.13, ADR-025); sus extensiones —interconexiones interregionales, baterías, ríos para la hidroeléctrica— siguen como expansión futura (GDD 22). El **CCRI-Flete y los slots de prioridad de terminales** (Fase 2) ya están **materializados** en el Incremento 8 (ver §5.11); su composición plena con el CCRI de venta queda como camino aditivo documentado. Las expansiones de la sección 22 del GDD (reputación, reserva compartida, futuros financieros, consorcios) son reintroducibles de forma aditiva sobre el CCRI sin rediseño.
