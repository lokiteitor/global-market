...
> **Nota de implementación (Incremento 8 — CCRI-Flete, sin cambio de diseño):** el CCRI-Flete se materializa como **segundo tipo de contrato** del Contract Service (`internal/contracts`), reutilizando **íntegra** la maquinaria del tablón del CCRI de bienes (ventana de sorteo, aceptación parcial, cursor, garantías, liquidación pro-rata) para el `publication_kind = freight`. **Flujo exacto:** el **cargador** publica la solicitud de flete y **bloquea su escrow** (precio del flete, como una compra) declarando el valor de la carga; el **transportista** la acepta con una **garantía** `II_FREIGHT_GUARANTEE_BP` (default 10 %) del valor declarado; al servirse la aceptación, `ledger.confirm_freight` mueve escrow + garantía a las cuentas espejo del contrato y **la carga a una cuenta `custody`** (asiento `custody_load`) — el transportista la lleva físicamente pero **el ledger le impide venderla** (no está en su `stock_free`). `world`/`fleet` materializa el cargamento del cargador y el **transportista** lo despacha (el despacho estándar se autoriza leyendo `carrier_account_id`); al entregar, el consumidor `freight_settler` liquida con `ledger.settle_freight_prorata`: la custodia va al cargador en el destino y el transportista **cobra el flete y recupera la garantía por lo entregado a tiempo**; el fallo por vencimiento libera la custodia in situ en el origen y **reparte la garantía** entre compensación al cargador (`II_FREIGHT_COMPENSATION_BP`, default 50 %) y sink. **Decisión de alcance (Fase 2):** se materializa por completo el flete **standalone** (el cargador mueve stock propio con custodia real); la **composición plena** con un CCRI de venta de un tercero es un **camino aditivo** —un cargamento que lleve `contract_id` y `freight_contract_id` dispara ambos liquidadores en su destino final, cada uno con su idempotencia—. Los **contratos privados** (5.3.1) usan el **mismo mecanismo** (canal `private`, ya v1): solo cambia el descubrimiento. Sin cambio de mecánica. Detalle en la sección *v1.8* de `documentacion_base_de_datos.md` y §5.11 de `arquitectura_imperio_industrial.md`.
...
> **Nota de implementación (Incremento 6b):** el faucet y el banco central algorítmico están materializados en el **Economy Balancer** (`internal/balancer`). El **faucet principal** es el pago de las ciudades a sus vendedores: la caja de la ciudad se **pre-fondea por emisión** (`+cash/−emission`) antes de publicar cada compra, así que una ciudad nunca incumple el pago. El **banco central algorítmico** es un lazo suave y acotado que mueve `tax_rate_bp`/`canon_base` un paso pequeño según la tendencia `masa monetaria − PIB` de `analytics.economy_indicators`, **nunca fuera de rango**. La coherencia es contable: `emisión − absorción = Δmasa monetaria` por bucket (`money_supply = cash+escrow+guarantee`). Sin cambio de mecánica. Detalle en la sección *v1.6* de `documentacion_base_de_datos.md` y §5.9 de `arquitectura_imperio_industrial.md`.
...
> **Nota de implementación (Incremento 6b — decisión de punto de entrega, sin cambio de diseño):** el **Economy Balancer** (`internal/balancer`) es el agente decisor de las ciudades. Recalcula la curva (`Demanda_efectiva`, precio por elasticidad de dos clases) con **todos los clamps obligatorios** —`supply_ema` con suelo `> 0`, `saturation_factor` acotado, `current_price ∈ [price_floor, price_ceiling]`—, corre la máquina de niveles (`supply_index` histórico → sube/baja de nivel, desbloquea categorías, decae por abandono logístico; emite `city.level_up`/`city.level_down`) y publica las buys de la ciudad **por la API estándar del Contract Service, sin canal privilegiado** (18.1). **Decisión de entrega vinculante:** para que la entrega estándar del CCRI deposite el stock como `stock_free` de la ciudad (que requiere un almacén), **cada ciudad tiene su propio edificio `distribution_center`** (infraestructura del sistema, `owner = ciudad`, sobre concesión del sistema), que es el **destino** de sus buys; al liquidarse, un consumer del Balancer **consume** lo entregado (`city stock_free → world_source`, ADR-022) para que la ciudad sea sumidero final real **sin acumular inventario**. Sin cambio de mecánica. Detalle en la sección *v1.6* de `documentacion_base_de_datos.md` y §5.9 de `arquitectura_imperio_industrial.md`.
...
> **Nota de implementación (Incremento 6b):** el **Economy Balancer** recalcula periódicamente el salario efectivo y lo **escribe en `world.cities.base_salary`** — `salario_base(nivel) × factor_saturación(ocupación_industrial_regional)`, con el multiplicador de saturación **acotado**—; el Balancer es su **única autoridad de escritura**. La ocupación proviene de `analytics.region_stats.industrial_occupation`, que el propio Balancer escribe en el mismo barrido. **Interacción documentada (a reconciliar):** el sink `wage` de producción (Incremento 2) lee `base_salary` y, en su forma actual, multiplica **además** por `industrial_occupation` (default 1.0); el estado final buscado es que producción lea `base_salary` **tal cual** (ya efectivo). Sin cambio de la fórmula de diseño (5.7). Detalle en la sección *v1.6* de `documentacion_base_de_datos.md`.
...
> **Nota de implementación (Incremento 10 — red eléctrica activada, ADR-025, sin cambio de diseño):** la red de Fase 3 está **materializada** con la especificación íntegra de este apartado y tres decisiones de implementación registradas en ADR-025. **(1) Convivencia por RECETA:** el combustible in situ sigue siendo el plan base y **ninguna receta existente cambia**; la electricidad entra como recetas nuevas (`world.recipes.power_per_hour`, unidades de energía por hora-sim mientras el lote está activo — p. ej. `smelt_steel_electric`, alternativa opt-in del alto horno) y el jugador se electrifica **eligiendo la receta activa**; una receta puede exigir combustible Y electricidad a la vez (conjunción); la activación es **orgánica por región** (el mercado existe donde hay líneas operativas; sin flag global). **(2) Recorte "por los que menos pagan":** cada consumidor tiene una **puja máxima** por unidad (`world.power_bids`, default `II_POWER_DEFAULT_BID_PRICE`) que fija la prioridad inversa del recorte y su techo personal (el cierre uniforme nunca la supera); entre pujas iguales el recorte **rota** prefiriendo al recortado menos recientemente (`last_curtailed_at_sim`). **(3) Conectividad de pool regional:** un edificio participa si está a ≤ `II_POWER_CONNECT_RADIUS_M` de una línea **operativa** de su región (el mercado es por región, como exige este apartado; sin flujos de potencia). Las **líneas** son infraestructura lineal propia **sin concesión de suelo** (cruzan muchas parcelas, como las carreteras): coste por longitud al sink y **mantenimiento por km/día-sim** cobrado por el motor de insolvencia con el patrón de edificios (impagado degrada; `abandoned` deja de conducir — el papel anti-acaparamiento del canon). Las **hidroeléctricas** restringen emplazamiento con la regla `requires_biome` (hoy bioma `coast`: los ríos del worldgen siguen pendientes; se extenderá a proximidad de río de forma aditiva). El **tick del spot** es del Economy Balancer (18.1): bucketizado en sim-time (`II_POWER_SPOT_INTERVAL_SIM`, default 1 hora-sim), liquida cada región en una tx con **un asiento `power_spot`** (cajas de consumidores → cajas de generadores al cierre uniforme; redistribución cash→cash, ni faucet ni sink) y el combustible de las térmicas como `consumption` contra `world_source` (ADR-022); **sin combustible no despachan**. El edificio sin suministro pausa con el estado nuevo `paused_no_power` (mismo 2º escalón de 5.9 que `paused_no_fuel`, con remedio distinto — por eso es estado propio, migración 0017). Detalle en la sección *v1.10* de `documentacion_base_de_datos.md` y §5.13 de `arquitectura_imperio_industrial.md`.
...
> **Nota de implementación (Incremento 7 — transporte multimodal, sin cambio de diseño):** los modos **ferroviario** (`rail`) y **marítimo** (`sea`) se activan con el mundo multi-región (sección 9): el generador tiende enlaces `rail` entre junctions terrestres adyacentes y `sea` entre regiones costeras/oceánicas, **partiendo cada enlace inter-región en la frontera en un segmento por región** (cada shard simula su lado, 15.1). El catálogo suma `freight_train`/`cargo_ship` (combustible `coal`), con una matriz coste/velocidad/volumen que hace el coste/unidad decreciente camión→tren→barco (sección 8). **Decisión de implementación del transbordo:** una ruta multimodal se recorre por **tramos de un solo modo con transbordo EXPLÍCITO por tramo** en una **terminal intermodal** (owner = banco central como infraestructura pública inicial; su propiedad por jugadores es el Incremento 8) — un vehículo solo circula por enlaces de su modo; al terminar un tramo, la carga con destino más allá **espera en la terminal** (estado `at_terminal`) hasta consumir el **tiempo de transbordo** (`transshipment_per_hour`) y ser re-despachada en un vehículo del siguiente modo—. Es exactamente lo que anticipa el bullet anterior ("rescate/transbordo … Fase 2"). Detalle en la sección *v1.7* de `documentacion_base_de_datos.md` y §5.10 de `arquitectura_imperio_industrial.md`.

> **Nota de implementación (Incremento 8 — slots de prioridad y colas de transbordo, sin cambio de diseño):** las **terminales del sistema** (owner = banco central, creadas por el generador donde coinciden `road` y `rail`/`sea`) **venden slots de prioridad**: `internal/worldgen` crea 3 slots a la venta por terminal (`priority_tier` 1..N, precio creciente con la prioridad) y `POST /world/terminal-slots/{slotId}/purchase` cobra `price` al **dueño de la terminal** (`cash→cash`) fijando `holder_account_id`+`valid_until_sim` (`II_SLOT_VALIDITY_SIM`, default 30 días-sim). **Materialización de las colas de transbordo:** al terminar un tramo, el cargamento entra en la **cola** de la terminal; el motor de tránsito la sirve con un modelo de **servidor único** a `transshipment_per_hour` en orden de **prioridad** —dueños con un slot vigente primero, `priority_tier` ascendente; el resto **FIFO** por llegada—, de modo que un cargamento con slot **queda listo antes** que uno sin slot llegado a la vez. Es la mecánica de "los transbordos consumen tiempo/capacidad y pueden generar colas" de este mismo apartado, con los slots como palanca de prioridad; los enlaces siguen siendo de uso común (sin reservas exclusivas de vía). Detalle en la sección *v1.8* de `documentacion_base_de_datos.md` y §5.11 de `arquitectura_imperio_industrial.md`.
...
- El pathfinding es **jerárquico** (estilo HPA*/contraction hierarchies): la grilla de macro-regiones actúa como nivel superior del grafo (rutas interregionales se planifican región-a-región y se refinan localmente), lo que evita ejecutar Dijkstra/A* plano sobre el grafo mundial en cada consulta. Los pesos usan la congestión **suavizada** (EMA) publicada por los shards, y la **replanificación solo ocurre en hitos** (llegada a nodo, evento de congestión severa), nunca de forma continua.
  - *Nota de implementación (Incremento 3, Fase 1 terrestre):* a la escala de la Fase 1 (una sola región, pocos nodos) el Logistics Service (`internal/logistics`) resuelve con **Dijkstra plano ponderado por congestión** (EMA), que es correcto y suficiente; la **jerarquía HPA*** queda diferida como **optimización por escala** (se activa cuando el grafo multi-región lo justifique) y no altera la arquitectura: la interfaz `Planner` la deja lista para insertar. Es la optimización que el propio párrafo enmarca ("evita Dijkstra plano sobre el grafo **mundial**"), no una decisión estructural nueva — no requiere ADR. **(Incremento 7):** con el grafo **multi-región** ya presente, el planificador **sigue resolviendo Dijkstra plano** —ahora sobre un grafo **expandido por `(nodo, modo)`** para el multimodalismo, con el **cambio de modo transitable solo en un nodo con terminal** y su tiempo de transbordo sumado a la ETA—; HPA* **sigue diferido** y se activará por **medición** a mayor escala.
...
> **Nota de implementación (Incremento 7 — mundo Fase 2, sin cambio de diseño):** la generación procedural la materializa el paquete `internal/worldgen` (binario `cmd/worldgen`, `make worldgen`), **determinista, idempotente y ADITIVO**: conserva intacta la región raíz **Askadia (0,0)** y su seed, y **añade** las regiones que la rodean en una **grilla `II_WORLD_GRID` centrada en (0,0)** (default **3×3** = las 8 regiones vecinas; nota de implementación, no de diseño). El determinismo es duro (1.1): un **value-noise 2D propio** (código del proyecto, sin dependencias nuevas) muestrea **elevación + humedad** en el centro de cada celda y una tabla de decisión asigna el **bioma** (`plains`/`forest`/`desert`/`mountain`/`coast`/`ocean`; costa y océano habilitan las rutas marítimas); un **RNG sembrado por celda** `(II_WORLD_SEED, gx, gy)` decide conteos y posiciones — **jamás** wall-clock/entropía, así que misma semilla ⇒ mismo mundo—. Cada región terrestre recibe **1-2 ciudades** (con su centro de distribución, caja prefondeada por emisión y demanda base, como las del Balancer) y **2-4 yacimientos** finitos con **recurso correlado al bioma** (por ahora solo `iron_ore`/`coal`; madera y petróleo de la sección 10 aún no están en el catálogo). **Ríos y temperatura** de esta sección quedan pendientes (biomas por elevación+humedad en el Incremento 7). Se genera **una vez y se persiste**. Detalle en la sección *v1.7* de `documentacion_base_de_datos.md` y §5.10 de `arquitectura_imperio_industrial.md`.
...
> **Nota de implementación (Incremento 9 — bots intermedios, sin cambio de diseño):** el `industrial_transformer` cubre el nivel "intermedio" con **reglas + optimización simple** pero **sin solver de programación lineal**: deriva el **coste unitario estimado** de la unidad producida a partir de las **cantidades de la receta** y del **precio de referencia OHLC** de cada insumo, exige un margen configurable sobre él (`II_BOTS_TRANSFORMER_MARGIN_BP`, default +25 %) y **para la cola** cuando el margen esperado es negativo; el `freighter` aplica el mismo esquema al viaje (tarifa contra combustible + opex + coste de oportunidad de la garantía, y ETA contra plazo). La sustitución es deliberada: una LP añadiría una dependencia y, sobre todo, **decisiones no auditables línea a línea**, mientras que este §13 exige que toda decisión de bot sea explicable (cada una emite log estructurado con motivo e ids + métrica). Si la calibración lo pidiera, la LP entra **detrás de la misma interfaz `Behavior`** sin cambiar nada más. Sin ML, en ningún nivel.
...
> **Nota de implementación (Incremento 9 — los tres modos materializados, sin cambio de diseño):** los tres modos ya son código y conviven como describe este apartado.
>
> - **Modo 1 «mundo vivo»**: el Bot Orchestration Service (`cmd/bots`) ejecuta de forma permanente los **cinco arquetipos** de 13.2 contra la API pública.
> - **Modo 2 «densidad dinámica»**: lo materializa el `DensityController` del orquestador, que ajusta **cuántos bots de cada arquetipo están activos** pausando y reanudando en caliente bots ya aprovisionados (conservan cuenta, capital y contratos; retirar es otra cosa, y solo por insolvencia). Señales por ciclo: **actividad humana** (sesiones vivas + comandos recientes: más humanos ⇒ se permite más densidad, hacen falta contrapartes), **saturación del sistema** (lag de la outbox y cola de transbordo: menos densidad) y **cobertura del tablón** (con pocas publicaciones vivas sube la densidad de los arquetipos de *backstop* de liquidez de 5.3.1 — productores y comerciante; el transformador y el transportista no crean mercado, lo necesitan). Cuando la saturación aparece, los bonos de actividad y cobertura **se descartan** y solo actúa el recorte: es literalmente la válvula de carga de la sección 19. **Dos precisiones de alcance en esta fase** (implementación, no diseño): el ajuste es **global, no por región** —el mundo corre en un proceso único y la señal de saturación también lo es (15.1/ADR-013); la partición por región llega con la extracción de shards, sin cambiar la fórmula—, y se modula el **número** de bots activos, no su «agresividad» (los umbrales de cada arquetipo siguen siendo configuración estable y auditable; modular la agresividad haría las heurísticas menos explicables, contra 13.3).
> - **Modo 3 «stress test»**: lo materializa el harness `cmd/stress`, un **cluster desacoplado y temporal** que juega por las **mismas APIs públicas** vía el SDK (15.4) con bots **ligeros de alta frecuencia** —generan carga sobre los caminos calientes, no juegan «bien»: el gameplay rico vive en los arquetipos del modo 1— y emite un informe con las mediciones. La expresión «instancias efímeras del motor» se materializa como **entorno de pruebas separado que el operador levanta** (mismo stack, base propia) contra el que se lanza el harness; la escala de «cientos de miles» se alcanza **horizontalmente**, lanzando varias instancias del harness (una corrida se acota a 200 000 bots por proceso para que la medición no mida al propio harness). La exigencia de "**no en el mundo de producción**" es **ejecutable**: el binario rehúsa arrancar sin `II_STRESS_API_URL` explícita (sin default), con `II_ENV` de producción, o si el host de la API **o el de la base de datos** no casan una allowlist de entornos no productivos; además prefija y retira sus cuentas (`stress-<run_id>-…`). Detalle operativo en `docs/guias/desarrollo.md` § *Stress test* y §5.12 de `arquitectura_imperio_industrial.md`.
