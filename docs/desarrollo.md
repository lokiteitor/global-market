...
| ADR-IMPL-11 | **Bots por la API pública real** (HTTP contra el gateway), sin atajos — la igualdad de API literal del GDD se verifica con ellos. Funcionamiento detallado en `docs/bots.md`. | GDD 13.1 | Principio de diseño intocable |
...
| ADR-IMPL-15 | **Vista isométrica v1 con assets pak128 (FE-6)**: `IsoProjection` afín 2:1 en el kernel del frontend (lon/lat → tiles continuos a 32 tiles/grado → px con rombo 128×64; norte arriba-derecha). Assets: pipeline offline `npm run build:assets` (`frontend/scripts/build-game-assets.mjs`, pngjs) que recorta frames curados de los tilesheets fuente de Simutrans en `frontend/app/assets/pak128/`, aplica el color clave RGB(231,255,255)→alpha, compone edificios multi-tile en un frame único y emite atlas Phaser + manifiesto de anclajes a `frontend/public/game/pak128/` (output commiteado). Render: terreno tileado por región con `Blitter` (sin chunks/streaming), carreteras rasterizadas celda a celda (supercover 4-conexo + máscara NSEW global → frame recta/curva/T/cruce), vehículos con sprites de 8 direcciones (ángulo px → frame, sin rotación), depth iso = base + y del pie del rombo. El culling del bridge y el bbox del viewport WS operan 100 % en lon/lat proyectando las 4 esquinas (la iso no mapea rects px ↔ lon/lat). | FAD §11.4, §14.7, §16.2 | El FAD preveía la iso en FE-6; esta v1 pragmática usa el pak128 (Artistic License 2.0, `ATTRIBUTION.txt` en el output) y deja chunks/streaming/LOD (FAD §16 completo) para Fase 2 |
...
- [x] Bots (productor, transformador, arbitrajista con heurísticas puras testeadas 36/36 — `docs/bots.md`)
