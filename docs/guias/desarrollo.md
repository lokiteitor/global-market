...
- **`shared/geometry/grid.ts` — GridProjection**: ÚNICO punto de conversión mundo ↔ pantalla (la API habla metros planos `[x_m, y_m]`, SRID 0). La ESCALA es fija (1 tile = 250 m = 32 px; chunks de 32 × 32 tiles); los LÍMITES del mundo son dinámicos desde el Incremento 13 (`WorldBoundsM`, unión de los bounds del catálogo de regiones vía `regionsBoundsM`; fallback Askadia `[0, 50 000)²` pre-catálogo; índices de tile/chunk negativos válidos al oeste/norte). Prohibida la matemática de proyección fuera de aquí.
...
**Convenciones añadidas (Incrementos 11–21)**: la planificación de rutas compartida vive en `app/composables/useRoutePlanning.ts` (despacho, reposicionamiento y estimación de trayecto del flete: no dupliques el flujo plan → ruta `on_demand`); los gráficos propios van en `app/components/play/charts/` con su **geometría pura en un módulo `.ts` testeado** y el pintado en el `.vue` (patrón `ohlc-layout.ts` + `OhlcChart.vue`; los importes se escalan a píxeles SOLO con `toApproxNumber` de `shared/money`); el minimapa es Canvas 2D en la capa Vue por ADR-026. Los eventos WS de fletes (`freight.*`) exigen contrato ≥ v1.7.0 (incremento 11).
