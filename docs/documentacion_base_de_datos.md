...
- **Ramal de última milla del alta de edificio (v1.9, Incremento 9):** `POST /world/buildings` no solo inserta el `world.network_nodes` del edificio: en la **misma transacción SERIALIZABLE** tiende un **ramal `road` bidireccional** (dos `network_links` dirigidos, uno por sentido, cada uno con su **único** `link_segments` en la región del edificio, `congestion_ema` 1.0) entre ese nodo y el **nodo road-conectado más cercano de su misma región**, con la misma `capacity_per_hour`/`base_speed_kmh` que la red del seed y del worldgen (60 uds/h, 80 km/h). Sin él, todo edificio construido en marcha —por un bot o por un humano— nacía **aislado**: `POST /world/vehicles` rechazaba la entrega (nodo inaccesible para el modo del vehículo) y el planificador no encontraba ruta, así que ni despachaba ni recibía nada. La carretera es **infraestructura del mundo**, no un activo de la corporación: no se cobra aparte del `build_cost` del tipo. Si la región todavía no tiene **ningún** nodo con enlace `road` (p. ej. una región oceánica), el alta **no falla**: el edificio se crea sin ramal y queda un `warn` en el log.
...
| `vehicle.repositioned` | `vehicle` | `world` | Viaje EN VACÍO: el vehículo idle arranca sin carga hacia el nodo final de una ruta propia (sin él quedaría varado donde terminó su última entrega) |
...
CREATE TABLE outbox.consumer_cursors (
    ...
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- v1.9 (0016_outbox_consumer_interest): tipos de evento suscritos, que el
    -- propio consumidor registra y refresca en cada polling.
...
- **Retraso de un consumidor (v1.9, migración `0016_outbox_consumer_interest`):** el retraso de un consumidor **NO es `max(seq)` menos su cursor**. Un consumidor solo avanza el cursor cuando procesa eventos **de sus tipos**, así que uno perfectamente sano suscrito a un evento raro (`system_liquidator` ← `building.seized`) se queda en `last_seq = 0` para siempre y esa resta devuelve el **total de eventos que el mundo ha emitido en su historia** —monótonamente creciente, jamás baja—. Por eso cada cursor **declara su suscripción** en `event_types` (la registra y refresca el propio consumidor en cada polling, `INSERT … ON CONFLICT DO UPDATE` solo si cambió: sin escrituras en régimen estacionario) y el retraso se mide como **eventos de esos tipos con `seq` por encima del cursor** (`ix_outbox_type_seq`). De ahí beben `ii_outbox_consumer_lag` (por consumidor, en el propio polling), `ii_outbox_lag_observed` (el peor consumidor; **válvula de carga de la densidad de bots**, GDD §19) y el `outbox_pending` del informe de stress. Un cursor sin suscripción declarada (`'{}'`) no aporta retraso: no se mide lo que no se sabe.
