-- =============================================================================
-- Imperio Industrial — 0002_auth (up)
-- Esquema auth: identidad, credenciales, sesiones y perfiles de bot.
-- Propiedad: módulo gateway del backend Go (ADR-017). Jugadores y bots
-- comparten el mismo modelo de cuenta (GDD 18.1 #1); ciudades y banco central
-- son cuentas de sistema sin canal privilegiado (Arquitectura §9).
-- IDs: uuid (UUIDv7 nativo, ADR-018).
-- =============================================================================
...
-- 2. account_credentials — secreto de autenticación por cuenta (hash argon2id,
--    golang.org/x/crypto; Security by Default, ADR-017). Separado de accounts
--    para que las lecturas de identidad nunca arrastren el hash.
CREATE TABLE auth.account_credentials (
    account_id    uuid PRIMARY KEY REFERENCES auth.accounts(id),
    secret_hash   TEXT NOT NULL,   -- codificación PHC de argon2id
    updated_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- 3. sessions — sesiones de cliente (wall-clock: única capa donde el tiempo
...
-- 4. bot_profiles — parámetros de comportamiento de la población de bots
