-- =============================================================================
-- Imperio Industrial — 0003_world (up)
...
-- IDs: uuid (UUIDv7 nativo, ADR-018). Geometrías: SRID 0 planar, unidad =
-- metro de mundo (ADR-019). Las FK de shipments hacia ledger se añaden en
-- 0004_ledger (cross-schema, cuando ledger exista).
-- =============================================================================
...
CREATE TABLE world.shipments (
    ...
    product_id           uuid NOT NULL REFERENCES world.products(id),
    quantity             stock_qty NOT NULL CHECK (quantity > 0),
    contract_id          uuid,   -- FK diferida a ledger.contracts (0004_ledger)
    freight_contract_id  uuid,   -- FK diferida a ledger.freight_contracts (0004_ledger)
...
-- 25. sim_clock — ancla persistida del reloj de simulación (GDD 1.1):
--     sim_time(t) = sim_time_at + (t - wall_anchor) * ratio, salvo frozen.
--     Fila única (id = 1); el motor la actualiza al re-anclar (arranque,
--     ventana de mantenimiento, cambios de ratio).
CREATE TABLE world.sim_clock (
    id           SMALLINT PRIMARY KEY CHECK (id = 1),
    sim_time_at  sim_time NOT NULL DEFAULT 0,
    wall_anchor  TIMESTAMPTZ NOT NULL DEFAULT now(),
    ratio        INT NOT NULL DEFAULT 24 CHECK (ratio > 0),  -- 1 s real = 24 s de juego (GDD 1.1)
